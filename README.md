...
1. 安装下面的软件/工具
    - 推荐安装Python 3.10+：https://www.python.org/downloads/windows/
...
3. 输入以下命令克隆本仓库并安装依赖
    ...
    ```powershell
    git clone https://github.com/a1046700338/HoshinoBot.git
    cd HoshinoBot
...
4. 回到资源管理器，进入`hoshino`文件夹，进入`config`文件夹，然后右键使用`sublime`打开其中的`__bot__.py`，按照其中的注释说明进行编辑。
